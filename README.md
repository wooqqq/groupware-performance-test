...
> nGrinder 100 vUser (Agent 2개 × 50) / 5분 부하 테스트 기준  
> 로컬 PC 1대에서 Spring Boot + nGrinder + DB + Redis를 동시 실행. v1/v2 상대적 개선폭이 핵심.

| 지표 | v1 (레거시) | v2 (개선) | 개선율 |
|------|------------|----------|--------|
| TPS | 280.2 | 309.5 | +10.5% |
| P95 응답시간 | 356ms | 102ms | **-71%** |
| Mean Test Time | 221ms | 79ms | **-64%** |
| 에러율 | 0% | 0% | - |
| DB 커넥션 활성 (평균) | 12.4개 | 1.52개 | **-88%** |
...
> 로컬 PC 1대에서 부하 발생기(nGrinder Agent)와 서버가 함께 실행되어 네트워크 큐가 포화된 결과입니다.  
> **서버 자체의 처리 신뢰도는 Grafana 에러율 0%로 확인합니다.**
>
> **v2 Errors(57,698)가 v1(32,931)보다 많은 이유**  
> v2는 응답이 빨라진 만큼 nGrinder가 같은 5분 안에 더 많은 요청을 시도했습니다 (v1: 115,412건 → v2: 148,830건, +28.8%).  
> 더 많은 요청이 로컬 네트워크 큐를 더 많이 포화시킨 결과로, v2가 더 잘 작동하기 때문에 생기는 역설적인 현상입니다.  
> 이 현상은 로컬 PC 1대에서 nGrinder와 서버를 함께 실행하는 구조적 한계로, nGrinder를 별도 서버에서 실행해야 해소할 수 있습니다.
